...
describe("getServer", () => {
	it("찾으면 날짜를 ISO로 직렬화해서 반환", async () => {
		...
		const result = await getServer(db, 'org-123', 1);
	...
	it("없으면 404 Error", async () => {
		...
		await expect(getServer(db, 'org-123', 999)).rejects.toMatchObject({
			statusCode: 404,
...
describe("createServer", () => {
	it("배열을 단건으로 unwrap", async () => {
		...
		const result = await createServer(db, 'org-123', {
			hostname: "solji-pc",
			ip: "192.168.10.51",
			status: 1,
		});
...
describe("updateServer", () => {
	it("빈 배열이면 404", async () => {
		...
		await expect(updateServer(db, 'org-123', 999, { status: 0 })).rejects.toMatchObject({
			statusCode: 404,
			code: "SERVER_NOT_FOUND",
			message: "Server with id 999 not found",
		});
...
describe("removeServer", () => {
	it("빈 배열이면 404", async () => {
		...
		await expect(removeServer(db, 'org-123', 999)).rejects.toMatchObject({
			statusCode: 404,
			code: "SERVER_NOT_FOUND",
			message: "Server with id 999 not found",
		});
